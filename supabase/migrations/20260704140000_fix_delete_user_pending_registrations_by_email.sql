-- Fix: safe_delete_user_with_receipts was only deleting pending_registrations by user_id.
-- Since pending_registrations records are inserted without a user_id (only email),
-- the DELETE WHERE user_id = target_user_id never removed the record.
-- This left a stale row in pending_registrations, causing the upsert on re-registration
-- to silently fail (unique constraint on email), leaving the email column NULL.
-- Fix: also delete pending_registrations by email after resolving it from user_profiles.
...
BEGIN
    -- Get user email for logging and for pending_registrations cleanup
    ...
    -- 5. ✅ FIX: Delete pending registrations by BOTH user_id AND email
    --    Previously only deleted by user_id, but records are inserted without user_id,
    --    so the old DELETE never removed the row. Now we also delete by email.
    BEGIN
    ...
    -- 11. Delete receipts associated with this user
    BEGIN
    ...
    -- 12. Delete instructor profiles (if user was an instructor)
    BEGIN
    ...
    -- 14. Delete from auth.users so the email is freed for re-registration
    BEGIN
