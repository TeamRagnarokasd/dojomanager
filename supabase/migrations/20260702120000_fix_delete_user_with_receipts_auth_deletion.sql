-- Location: supabase/migrations/20260702120000_fix_delete_user_with_receipts_auth_deletion.sql
-- Fix: safe_delete_user_with_receipts was not deleting the user from auth.users
-- This caused the email to remain "occupied" in Supabase Auth even after full deletion,
-- preventing re-registration with the same email.
...
BEGIN
    ...
    -- 14. ✅ FIX: Delete from auth.users so the email is freed for re-registration
    BEGIN
