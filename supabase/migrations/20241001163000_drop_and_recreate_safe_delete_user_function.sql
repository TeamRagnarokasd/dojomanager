-- Fix safe_delete_user function by dropping and recreating
-- The error "cannot change name of input parameter" occurs when trying to modify parameter names
-- Solution: DROP the function first, then CREATE with new parameter name

-- Drop the existing function first (required for parameter name changes)
DROP FUNCTION IF EXISTS public.safe_delete_user(uuid);

-- Recreate the function with correct parameter name and fixed column references
...
BEGIN
    ...
    -- 11. FIXED: Clean up admin activity log references - NO ambiguity with new parameter name
    -- Update target_user_id references
    ...
    UPDATE public.admin_activity_log 
    SET admin_id = NULL 
    WHERE admin_id = user_id_to_delete;
...
COMMENT ON FUNCTION public.safe_delete_user(uuid) IS 'Safely deletes a user and all associated data while preserving receipts/invoices. Function recreated with parameter renamed from target_user_id to user_id_to_delete to resolve ambiguous column references.';