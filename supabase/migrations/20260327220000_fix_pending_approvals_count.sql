-- Fix get_dashboard_statistics: pending_approvals should not count registrations
-- where the user already has an approved status in user_profiles.
-- This prevents the counter from showing stale "pending" records after approval.
...
BEGIN
    ...
    -- a corresponding user_profiles entry with status = 'approved'.
    -- Also count user_profiles with status = 'pending' (direct registrations).
    SELECT (
        -- Pending registrations not yet approved in user_profiles
        SELECT COUNT(*)
...
'Returns dashboard statistics for admin users. Uses SECURITY DEFINER to bypass RLS policies. Fixed pending_approvals to exclude already-approved users.';
