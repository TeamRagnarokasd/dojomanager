-- Location: supabase/migrations/20241001161248_fix_safe_delete_user_ambiguity.sql
-- Schema Analysis: Existing safe_delete_user function has parameter name conflicts and column ambiguity
-- Integration Type: Modification - Fix existing function by dropping and recreating
-- Dependencies: admin_activity_log, user_profiles tables

-- First drop the existing function to allow parameter name changes
DROP FUNCTION IF EXISTS public.safe_delete_user(uuid);

-- Recreate the function with fixed column qualification and corrected logic
...
BEGIN
    ...
    -- 11. FIXED: Clean up admin activity log references with qualified column names and separate operations
    UPDATE public.admin_activity_log 
    SET admin_activity_log.target_user_id = NULL 
    WHERE admin_activity_log.target_user_id = target_user_id;
    GET DIAGNOSTICS records_deleted = ROW_COUNT;
    
    UPDATE public.admin_activity_log 
    SET admin_activity_log.admin_id = NULL 
    WHERE admin_activity_log.admin_id = target_user_id;
    GET DIAGNOSTICS records_deleted = ROW_COUNT;
