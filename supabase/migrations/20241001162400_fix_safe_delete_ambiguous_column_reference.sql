-- Fix ambiguous column reference in safe_delete_user function
-- Issue: target_user_id parameter conflicts with column name in admin_activity_log table
...
BEGIN
    ...
    -- 11. FIXED: Clean up admin activity log references - NO MORE AMBIGUITY
...
-- Add helpful comment
COMMENT ON FUNCTION public.safe_delete_user(uuid) IS 'Safely deletes a user and all associated data while preserving receipts/invoices. Parameter renamed from target_user_id to user_id_to_delete to avoid column name ambiguity.';