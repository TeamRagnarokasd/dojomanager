-- Location: supabase/migrations/20250928170000_add_user_deletion_functionality.sql
-- Schema Analysis: User management system with existing user_profiles table and related data
-- Integration Type: extension - adding user deletion functionality
-- Dependencies: user_profiles, admin_activity_log, non_fiscal_receipts, payment_confirmations, user_subscriptions, instructor_profiles, subscription_entry_usage, user_session_activity, admin_sessions, admin_communications, app_sessions, pending_registrations

-- Function to safely delete user and all related data except invoices/receipts
...
BEGIN
    ...
    DELETE FROM public.admin_sessions WHERE user_id = target_user_id;
    GET DIAGNOSTICS records_deleted = ROW_COUNT;
    total_deleted := total_deleted + records_deleted;
    ...
    WHERE instructor_id IN (
        SELECT id FROM public.instructor_profiles WHERE user_id = target_user_id
    );
    GET DIAGNOSTICS records_deleted = ROW_COUNT;
    total_deleted := total_deleted + records_deleted;
    ...
    DELETE FROM public.admin_communications WHERE sender_id = target_user_id;
    GET DIAGNOSTICS records_deleted = ROW_COUNT;
    total_deleted := total_deleted + records_deleted;
    ...
    -- 11. Clean up admin activity log references (nullify, don't delete for audit trail)
    ...
    -- 13. Delete from auth.users (last, after all references are cleaned)
...
-- Create admin verification function if it doesn't exist
